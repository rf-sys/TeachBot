class ConversationFormNewParticipant extends React.Component {
    constructor(props) {
        super(props);
    ...
    addParticipant(user) {
        let ajax = $.post(`/chats/${this.props.conversation}/add_participant`, {user_id: user.id});
        ajax.done(() => {
    ...
    render() {
        ...
        let usersList = this.state.users.map((user) => {
            ...
            if (!no_participant) {
                return <ConversationFormFoundUser user={user} key={user.id} addParticipant={this.addParticipant}/>
            }
        });
