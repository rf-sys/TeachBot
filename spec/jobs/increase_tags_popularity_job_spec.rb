...
RSpec.describe IncreaseTagsPopularityJob, type: :job do
  include ActiveJob::TestHelper

  subject(:job) { described_class.perform_later(%w(tag1 tag2)) }
  ...
  it 'job matches the enqueued job' do
    expect { job }.to have_enqueued_job(IncreaseTagsPopularityJob)
  end

  it 'has appropriate arguments' do
    expect { job }.to have_enqueued_job.with(%w(tag1 tag2))
  end
  ...
  describe 'behaviour' do
    before :each do
      @redis = Redis.new
      @redis.del(RedisGlobals.popular_tags)
  ...
  def launch_job(tags)
    described_class.perform_now tags
  end
